...
public abstract class BleScanPresenter implements BluetoothAdapter.LeScanCallback {
    ...
    private void checkDevice(BleDevice bleDevice) {
        if (TextUtils.isEmpty(mDeviceMac) && (mDeviceNames == null || mDeviceNames.length < 1)) {
            System.out.println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$Had to correct Device");
    ...
    private void correctDeviceAndNextStep(final BleDevice bleDevice) {
        if (mNeedConnect) {
            ...
            if (!hasFound.get()) {
                System.out.println("The device wasn't found");
                ...
                if (bleDevice.getName() != null) {
                    System.out.println(bleDevice.getName().toString());
                String deviceName = new String(bleDevice.getName().toString());
                String actualName = "Accelerometer2";
                if (deviceName.equals(actualName) ) {
                    System.out.println("FOUND ACCELEROMETER 2");
                    mBleDeviceList.add(bleDevice);
                    mMainHandler.post(new Runnable() {
                        @Override
                        public void run() {
